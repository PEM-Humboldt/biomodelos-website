class RatingsController < ApplicationController
	...
	def destroy
		@rating = Rating.where(:model_id => params[:model_id], :user_id => current_user.id).first
		if !@rating.blank?
			@rating.destroy
		end
	...
	def rate_model
		...
		@rating = Rating.where(:model_id => params[:model_id], :user_id => current_user.id).first
		if @rating.blank?
			Rating.create({:model_id => params[:model_id], :user_id => current_user.id, :species_id => params[:species_id], :score => params[:score]})
		else
