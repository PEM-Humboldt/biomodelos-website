class Group < ActiveRecord::Base
	has_many :groups_users
  	...
  	def activity
  		Group.find_by_sql(
  			"SELECT * FROM(
				SELECT u.id, u.name user_name,'0' sp_id, '' task_name, gu.created_at updated_at, 'new_user' type
     			FROM users u, groups_users gu
     			WHERE u.id = gu.user_id AND gu.groups_users_state_id = 1 AND gu.group_id = "+self.id.to_s+"
     			ORDER BY gu.created_at DESC 
     			LIMIT 15) NUEVOS_USUARIOS
     		UNION ALL
	 		SELECT * FROM(
     			SELECT u.id, u.name user_name, t.species_id sp_id, tt.name task_name, t.updated_at, 'task_done' type
