$(document).ready(function(){
	...
	* @param {string} origin - origin of the action: group or user.
	*/
	function _markFinishedTask(task_id, origin){
	...
	*/
	$("body").on("change", "#group_tasks_table input[type=checkbox]", function () {
		...
		_markFinishedTask(task_id, "group");
  	});

  	$("body").on("change", "#user_tasks_table input[type=checkbox]", function () {
		var task_id = $(this).val();
		_markFinishedTask(task_id, "user");
  	});
});
